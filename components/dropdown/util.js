...
export function positionDropdown({
  ...
  // Set X position, etc:
  if (unfold === 'right') {
    setStyle({ left: `${left}px`, right: null })
  } else if (unfold === 'left') {
    setStyle({ left: `${right}px` })
  } else if (unfold === 'center') {
    setStyle({ left: `${left + wOf$opener/2}px`, right: null })
  }

  if (shouldSetMinWidth) {
    setStyle({ minWidth: `${Math.max(wOf$opener, wOf$menu)}px` })
  }
